...
const OtherInfoSection = ({ employee, setEmployee, onPrevious, onNext }) => {
  ...
  const hasCriminalCase =
    watch("has_criminal_case") === true ||
    watch("has_criminal_case") === "true";
  const hasAdminOffense =
    watch("has_admin_offense") === true ||
    watch("has_admin_offense") === "true";
  const wasSeparatedEmployment =
    watch("was_separated_employment") === true ||
    watch("was_separated_employment") === "true";
  ...
  useEffect(() => {
    ...
    reset({
      ...
      was_separated_employment: otherInfo.was_separated_employment
        ? "true"
        : "false",
  ...
  const handleSaveChanges = async (data) => {
    try {
      const payload = {
        hasCriminalCase:
          data.has_criminal_case === true || data.has_criminal_case === "true",
        criminalCaseDetails:
          data.has_criminal_case === true || data.has_criminal_case === "true"
            ? data.criminal_case_details
            : "",

        hasAdminOffense:
          data.has_admin_offense === true || data.has_admin_offense === "true",
        adminOffenseDetails:
          data.has_admin_offense === true || data.has_admin_offense === "true"
            ? data.admin_offense_details
            : "",

        wasSeparatedEmployment:
          data.was_separated_employment === true ||
          data.was_separated_employment === "true",
        separationDetails:
          data.was_separated_employment === true ||
          data.was_separated_employment === "true"
            ? data.separation_details
            : "",
      };
      ...
      toast.error(
        err?.response?.data?.message || "Failed to save other information",
      );
      return false;
    }
  };
